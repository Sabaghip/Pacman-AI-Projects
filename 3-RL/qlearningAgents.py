...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action1 in legalActions:
          if self.getQValue(state, action1) > max:
            max = self.getQValue(state, action1)
            action = action1
        return action
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        sample = reward + self.discount * self.getQValue(nextState, bestAction)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        fs = self.featExtractor.getFeatures(state,action)
        for f in fs:
          self.weights[f] += d * self.alpha * fs[f]
        return
        util.raiseNotDefined()
