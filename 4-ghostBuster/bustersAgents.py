    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        #print(f"pacPos = {pacmanPosition}, legal = {legal}, livingGhosts = {livingGhosts}, ghoPos = {livingGhostPositionDistributions}")
        temp = []
        x = 0 
        for i in range(len(livingGhosts)):
            if livingGhosts[i]:
                max = 0
                pos = (0, 0)
                for j in self.ghostBeliefs[x].keys():
                    if livingGhostPositionDistributions[x][j]>max:
                        max = livingGhostPositionDistributions[x][j]
                        pos = j
                temp.append(pos)
                x += 1
            else:
        ...
        for action in legal:
            ...
            for i in temp:
                if i != -1:
                    d = self.distancer.getDistance(nextPos, i)
                    if d<min:
                        min = d
                        bestAction = action
        return bestAction
